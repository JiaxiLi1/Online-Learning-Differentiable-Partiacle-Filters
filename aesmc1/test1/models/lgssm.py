...
class TrainingStats(object):
    def __init__(self, initial_loc, initial_scale, true_transition_mult,
        ...
        #torch.diag(self.true_transition_mult if self.dim != 1 else
                                               #self.true_transition_mult.unsqueeze(-1)).squeeze()
        ...
        for test_obs_idx in range(num_test_obs):
            ...
            self.true_posterior_means[test_obs_idx] = np.reshape(
                lgssm_true_posterior(observations, initial_loc, initial_scale,
                                     self.true_transition_mult, torch.zeros_like(initial_loc),
                                     transition_scale, self.true_emission_mult,
                                     torch.zeros_like(initial_loc), emission_scale)[0],
                (-1,))
        ...
        self.true_posterior_means_test = [None] * num_test_obs
        for test_obs_idx in range(num_test_obs):
            latent_test = [[l[test_obs_idx]] for l in self.test_obs_test[0]]
            observations_test = [[o[test_obs_idx]] for o in self.test_obs_test[1]]
            self.true_posterior_means_test[test_obs_idx] = np.reshape(
                lgssm_true_posterior(observations_test, initial_loc, initial_scale,
                                     self.true_transition_mult, torch.zeros_like(initial_loc),
                                     transition_scale, self.true_emission_mult,
                                     torch.zeros_like(initial_loc), emission_scale)[0],
                (-1,))
    ...
    def __call__(self, epoch_idx, epoch_iteration_idx, loss, initial,
                 transition, emission, proposal, test=False):
        if test == True:
            ...
            posterior_means = aesmc.statistics.empirical_mean(
                torch.cat([latent.unsqueeze(-1) for
                           latent in
                           [original_latents.cpu() for original_latents in inference_result['original_latents']]],
                          dim=2),
                inference_result['log_weight'].cpu()).detach().numpy()
            posterior_error_test = np.mean(np.linalg.norm(
                self.true_posterior_means - posterior_means.squeeze(), axis=1))
            loss_rmse_test=inference_result['loss_report'].cpu().detach().numpy()
            print('posterior_error:', posterior_error_test, 'loss_rmse_test:', loss_rmse_test)
            return np.array(posterior_error_test),loss_rmse_test
