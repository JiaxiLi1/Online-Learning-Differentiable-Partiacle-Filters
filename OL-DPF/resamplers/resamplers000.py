...
class resampler(nn.Module):
    def __init__(self, param):
        ...
        if param.resampler_type=='ot':
            self.kargs={'eps':param.epsilon, 'scaling':param.scaling, 'threshold':param.threshold,'max_iter':param.max_iter, 'device':param.device}
            self.resampling=resampler_ot
        elif param.resampler_type=='soft':
            self.kargs = {'index':True,'alpha':param.alpha, 'device':param.device}
            self.resampling=soft_resampler
    def forward(self, particles, particle_probs):
        particles_resampled, particle_probs_resampled = self.resampling(particles, particle_probs, **self.kargs)
        return particles_resampled, particle_probs_resampled


def soft_resampler(particles, particle_probs, alpha, index=True, device='cuda'):
    assert 0.0 < alpha <= 1.0
    batch_size, num_resampled = particles.shape[0], particles.shape[1]
    ...
    particles_resampled = particles.reshape((batch_size * num_resampled, -1))[idx, :]
...
def resampler_ot(particles, weights, eps=0.1, scaling=0.75, threshold=1e-3, max_iter=100
    ...
    #index_p = (torch.arange(num_particles)+num_particles* torch.arange(batch_size)[:, None].repeat((1, num_particles))).type_flow(torch.int64).to(device)
...
class transport(torch.autograd.Function):
    @staticmethod
    def forward(ctx, x, logw, eps, scaling, threshold, max_iter, n, device='cuda'):
        ctx.x=x.detach().clone().requires_grad_()
        ctx.logw=logw.detach().clone().requires_grad_()
        ctx.eps, ctx.scaling, ctx.threshold, ctx.max_iter, ctx.n = eps, scaling, threshold, max_iter, n
    ...
    def backward(ctx, d_transport):
        ...
        x_, logw_ = ctx.x, ctx.logw
        dx, dlogw = transport_grad(x_, logw_, ctx.eps, ctx.scaling, ctx.threshold, ctx.max_iter, ctx.n, grad_output=d_transport)
        return dx, dlogw, None, None, None, None, None, None
...
def OT_resampling(x, logw, eps, scaling, threshold, max_iter, n, device='cuda',
    ...
    transport_matrix = calculate_transport(x, logw, eps, scaling, threshold, max_iter, n)
