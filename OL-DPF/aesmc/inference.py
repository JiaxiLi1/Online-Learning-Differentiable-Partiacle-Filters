...
def infer(online_data, inference_algorithm, observations, initial, transition, emission,
    ...
    if online_state == 'start' or online_state == None:
        # if args.experiment == 'position_tracking':
        ...
        #     tensor_shape = (batch_size, num_particles)
        #     # if online_state == None:
        #     #     latent[:, :, 0] = torch.distributions.Normal(0, 1.0).sample(tensor_shape)
        #     #     latent[:, :, 1] = torch.distributions.Normal(0, 1.0).sample(tensor_shape)
        #     #     latent[:, :, 2] = torch.distributions.Normal(38.8908, 1.0).sample(tensor_shape)
        #     #     latent[:, :, 3] = torch.distributions.Normal(38.8908, 1.0).sample(tensor_shape)
        #     #     latent[..., -1] = torch.distributions.Normal(5 / 10.4881, 1.0).sample(tensor_shape)
        #     # elif online_state == 'start':
        #     #     latent[:, :, 0] = torch.distributions.Normal(0, 1.0).sample(tensor_shape)
        #     #     latent[:, :, 1] = torch.distributions.Normal(0, 1.0).sample(tensor_shape)
        #     #     latent[:, :, 2] = torch.distributions.Normal(38.8908, 1.0).sample(tensor_shape)
        #     #     latent[:, :, 3] = torch.distributions.Normal(38.8908, 1.0).sample(tensor_shape)
        ...
        if inference_algorithm == 'smc':
            if args.resampler_type == 'ot' or args.resampler_type == 'soft':
                ...
                particle_weights = math.normalize_log_probs(weights_pre)+1e-8
