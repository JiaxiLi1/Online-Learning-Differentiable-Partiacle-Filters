...
def build_encoder_maze(state_dim, dropout_keep_ratio):
...
class Proposal_cnf(nn.Module):
    ...
    def sample(self, previous_latents=None, time=None, observations=None, batch_size = 10, num_particles = 100):
        self.scale_0 = torch.diag(self.scale_0_vector.exp() if self.dim != 1 else self.scale_0_vector.unsqueeze(-1)).squeeze()
        self.scale_t = torch.diag(self.scale_t_vector.exp() if self.dim != 1 else self.scale_t_vector.unsqueeze(-1)).squeeze()
...
def build_dyn_nf(n_sequence, hidden_size, state_dim, init_var=0.01, translate = False):
...
class TrainingStats(object):
    def __init__(self, true_transition_mult_online1, true_emission_mult_online1, true_transition_mult_online2, true_emission_mult_online2, initial_loc, initial_scale, true_transition_mult,
        ...
        _, dataloader = train.get_synthetic_dataloader(self.initial,
                                                          self.true_transition,
                                                          self.true_emission,
                                                          num_timesteps,
                                                          num_test_obs, 10, 100, self.dim)
        ...
        # dataloader_online1 = aesmc.train.get_synthetic_dataloader(self.initial,
        ...
        # dataloader_online2 = aesmc.train.get_synthetic_dataloader(self.initial,
    ...
    def __call__(self,initial_states, epoch_idx, epoch_iteration_idx, loss, initial,
        ...
        if epoch_iteration_idx % self.saving_interval == 0 or epoch_iteration_idx + 1 == self.num_iterations:
            ...
            if stage == 0:
                ...
                inference_result, _, _ = inference.infer(
                    [None,None, initial_states], self.algorithm, self.test_obs[1], self.initial,
                    transition, emission, proposal,
