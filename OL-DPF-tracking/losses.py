...
def get_loss(normalising_value, mask, initial_states, observations, num_particles, algorithm, initial, transition,
    ...
    inference_result, data_pre_step, rmse_list, predicted_state, ground_truth, particles, normalized_particle_weights = inference.infer(normalising_value=normalising_value,
                                                      mask=mask,
    ...
    loss_rmse = args.rmse_ratio * inference_result['loss_rmse']
    loss = -args.elbo_ratio * torch.mean(elbo)
    loss_pl = args.pl_ratio * inference_result['pseudo_loss']
    # print(loss, loss_rmse)

    if args.trainType == 'dpf':
        return loss_rmse, data_pre_step, rmse_list, torch.mean(elbo), inference_result['loss_report'], predicted_state, ground_truth, particles, normalized_particle_weights
    elif args.trainType == 'dpf_elbo':
        return loss_rmse + loss, data_pre_step, rmse_list, torch.mean(elbo), inference_result['loss_report'], predicted_state, ground_truth, particles, normalized_particle_weights
    elif args.trainType == 'dpf_pl':
        return loss_rmse + loss_pl, data_pre_step, rmse_list, torch.mean(elbo), inference_result['loss_report'], predicted_state, ground_truth, particles, normalized_particle_weights
    else:
