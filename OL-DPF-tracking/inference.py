...
def infer(normalising_value, mask, initial_states, inference_algorithm, observations, initial, transition, emission,
    ...
    for time in range(1, len(observations)):
        ...
        if measurement == 'CRNVP':
            encodings = emission.obs_encoder(observations[time])
        ...
        log_weights.append(transition_log_prob + emission_log_prob - proposal_log_prob)
    ...
    normalized_particle_weights = aemath.normalize_log_probs(torch.stack(log_weights, dim=0))
    ...
    particles = torch.stack(original_latents, dim=0)
    predicted_state = torch.sum(particles * normalized_particle_weights, keepdim=True, dim=2)
    ...
    if mask.sum() != 0:
        loss_rmse = torch.mean(torch.sum(diff ** 2, dim=-1))
    else:
        loss_rmse = torch.tensor(0).to(diff.device)
    ...
    return {'log_marginal_likelihood': log_marginal_likelihood,
            ...
            'loss_report': loss_report}, [latents_bar[-1].detach(), log_weights[-1].detach()], rmse_list, predicted_state[:, 0, 0, 0:2], ground_truth[:, 0, :, 0:4], particles[:, 0, :, 0:4], normalized_particle_weights[:, 0, :, 0]
