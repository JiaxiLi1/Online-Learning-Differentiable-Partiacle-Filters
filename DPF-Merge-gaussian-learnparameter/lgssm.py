...
class Proposal_cnf(nn.Module):
    def __init__(self, transition, initial, scale_0, scale_t, type = 'planar', k=2):
        ...
        self.lin_0 = nn.Linear(self.dim, self.dim, bias=False)
        self.lin_t = nn.Linear(2*self.dim, self.dim, bias=False)

        self.type_flow = type
        if self.type_flow == 'nvp':
            self.k = k
            self.nvp = RealNVP_cond_compose(k=k, dim = self.dim, hidden_dim= 2 * self.dim, obser_dim = self.dim)
    ...
    def sample(self, previous_latents=None, time=None, observations=None, batch_size = 10, num_particles = 100):
        ...
        if self.type_flow != 'bootstrap':
            if time == 0:
                loc = self.lin_0(observations).squeeze(-1)
                ...
                loc = self.lin_t(torch.cat(
                        [previous_latents.unsqueeze(-1) if self.dim ==1 else previous_latents,
                         (observations.view(-1, 1, 1) if self.dim ==1 else observations.unsqueeze(1)).repeat(1, num_particles, 1)],
                        dim=2
                    ).view(-1, 2*self.dim)).squeeze(-1).view((-1, num_particles) if self.dim ==1 else (-1, num_particles, self.dim))
...
class RealNVP_cond_t(nn.Module):
    ...
    def forward(self, x, obser, time):
        ...
        # x = (x-x_mean)/x_std
        obser = obser.unsqueeze(1).repeat(1, x.shape[1], 1)
