...
functional_derivatives = {
    ...
    F.leaky_relu: lambda x: (x > 0).type(torch.cuda.FloatTensor) + \
                            (x < 0).type(torch.cuda.FloatTensor) * -0.01,
    F.elu: lambda x: (x > 0).type(torch.cuda.FloatTensor) + \
                     (x < 0).type(torch.cuda.FloatTensor) * torch.exp(x)
}

device = torch.device('cuda') if torch.cuda.is_available() else torch.device('cpu')

class Planar(nn.Module):
    def __init__(self, dim, obser_dim, nonlinearity=torch.tanh, device='cuda'):
        super().__init__()
        self.h = nonlinearity
        self.w = nn.Parameter(torch.Tensor(dim))
        self.u = nn.Parameter(torch.Tensor(dim))
        self.b = nn.Parameter(torch.Tensor(obser_dim))

        # self.lin_b = nn.Linear(dim, dim)
        self.dim = dim

        self.zero_initialization(dim)
    ...
    def zero_initialization(self, var=0.1):
        ...
        init.uniform_(self.u, -1/100, 1/100)
        init.uniform_(self.b, -math.sqrt(1/dim), math.sqrt(1/dim))

    def inverse(self, x, observation):
        ...
        u = self.u
        lin = torch.unsqueeze( x @ self.w, -1) + (self.b * observation.clone().detach()).sum(-1, keepdims=True)
        z = x + u * self.h(lin)
