...
def save_data(data_list, name_list, algorithm, lr, num_exps, num_iterations, dim, labelled_ratio, trainType):
    dir_name = f'./sto_test_autoencoder_plots_{algorithm}_{lr}_{num_exps}_{num_iterations}_{dim}_{trainType}_{labelled_ratio}/'
...
class TestModels(unittest.TestCase):
    ...
    def test_lgssm(self,args):
        ...
        from models import lgssm_sto as lgssm
        ...
        initial_loc = torch.tensor(0.9).to(device).squeeze()
        initial_scale = torch.tensor(0.5 ** 2 / (1 - 0.5 ** 2)).to(device).squeeze()
        ...
        true_transition_scale = (0.5 * torch.ones(dim)).to(device).squeeze()
        true_transition_scale_online = (0.8 * torch.ones(dim)).to(device).squeeze()
        init_transition_scale = (1.0 * torch.ones(dim)).to(device).squeeze()

        init_proposal_scale_0 = 0.1 * torch.ones(dim).to(device)  # (0.1*torch.eye(dim)).to(device).squeeze()
        init_proposal_scale_t = 0.1 * torch.ones(dim).to(device)  # (0.1*torch.eye(dim)).to(device).squeeze()


        emission_mult = torch.zeros(dim).to(device).squeeze()
        ...
        dataloader = train.get_synthetic_dataloader(
            ...
            lgssm.Transition(true_transition_mult, true_transition_scale).to(device),
            lgssm.Emission(emission_mult).to(device),
            num_timesteps, batch_size)

        dataloader_online = train.get_synthetic_dataloader_online(
            lgssm.Initial(initial_loc, initial_scale).to(device),
            lgssm.Transition(true_transition_mult_online, true_transition_scale_online).to(device),
            lgssm.Emission(emission_mult).to(device),
        ...
        for algorithm in algorithms:
            ...
            for i in range(num_experiments):
                ...
                Transition_dist = lgssm.Transition(init_transition_mult,
                                                        init_transition_scale).to(device)
                Emission_dist = lgssm.Emission(emission_mult).to(device)
                markers = {'aesmc': 'X',
                           'bootstrap': 'p',
                           'cnf-dpf-1': '*',
                           'cnf-dpf-nvp-2': '^',
                ...
                elbo_recorder.append(training_stats.loss_history)
                ESS_recorder.append(1/((np.array(training_stats.normalized_log_weights_history)**2).sum(-1)).mean(-1))
                print('Exp. {}/{}, {}, parameter error:{:.6f}+-{:.6f}, ELBO:{:.6f}+-{:.6f}, ESS:{:.6f}+-{:.6f}'
        ...
        for ax in axs:
            for x in ax:
                x.grid(alpha=0.5)
        fig.tight_layout()
        filename = './sto_test_autoencoder_plots/lgssm_elbo.pdf'
        fig.savefig(filename, bbox_inches='tight')
        print('\nPlot saved to {}'.format(filename))
