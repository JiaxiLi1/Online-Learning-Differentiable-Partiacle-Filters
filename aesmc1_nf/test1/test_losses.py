...
class TestModels(unittest.TestCase):
    ...
    def test_lgssm(self,args):
        ...
        if dim > 1:
            ...
            true_transition_mult = (0.9 * torch.ones(dim)).to(device).squeeze()
            init_transition_mult = (0.0 * torch.ones(dim)).to(device).squeeze()

        transition_scale = torch.eye(dim).to(device).squeeze()
        true_emission_mult = (0.5 * torch.ones(dim)).to(device).squeeze()
        init_emission_mult = (0.0 * torch.ones(dim)).to(device).squeeze()
        ...
        for algorithm in algorithms:
            ...
            posterior_error_test, loss_rmse_test = training_stats(-1, -1, -1, initial=Initial_dist,
                                                                  transition=Transition_dist, emission=Emission_dist,
                                                                  proposal=proposal, test=True)
            data_list_test = [posterior_error_test,
                              loss_rmse_test]
            data_name_list_test = ['posterior_error_test',
                                   'loss_rmse_test']
