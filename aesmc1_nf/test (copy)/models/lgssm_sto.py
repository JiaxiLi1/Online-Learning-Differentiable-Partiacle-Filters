...
class Emission(nn.Module):
    ...
    def forward(self, latents=None, time=None, previous_observations=None):
        loc = torch.diag(self.mult).squeeze()
        ...
        if epoch_iteration_idx % self.saving_interval == 0 or epoch_iteration_idx + 1 == self.num_iterations:
            ...
            if online == False:
                self.p_l2_history.append(np.linalg.norm(
                    np.array([transition.mult.flatten().cpu().detach().numpy(), transition.scale.flatten().cpu().detach().numpy()],dtype=object)-
                                                  np.array([self.true_transition_mult.cpu().detach().numpy(), self.true_transition_scale.cpu().detach().numpy()],dtype=object)
                ).squeeze())
            else:
                self.p_l2_history.append(np.linalg.norm(
                    np.array(
                        [transition.mult.flatten().cpu().detach().numpy(), transition.scale.flatten().cpu().detach().numpy()],
                        dtype=object) -
                                          np.array([0.3,
                                                    0.8], dtype=object)
                ).squeeze())
