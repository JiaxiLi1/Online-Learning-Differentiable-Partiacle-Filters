...
class TrainingStats(object):
    def __init__(self, initial_loc, initial_scale, true_transition_mult,
        ...
        self.true_transition = Transition(torch.tensor(0.3),
                                          transition_scale).to(device)
        self.true_emission = Emission(torch.tensor(0.8), emission_scale).to(device)
        ...
        #test stage
        self.true_transition_test = Transition(torch.tensor(0.3).to(device),
                                          transition_scale).to(device)
        self.true_emission_test = Emission(torch.tensor(0.8).to(device), emission_scale).to(device)
